...
def things(x_coord, y_coord, width, height, color):
    pygame.draw.rect(gameDisplay, color, [x_coord, y_coord, width, height])
...
def button(message, x_coord, y_coord, width, height, inactive_color, active_color, action=None):
    ...
    if x_coord + width > mouse[0] > x_coord and y_coord + height > mouse[1] > y_coord:
        pygame.draw.rect(gameDisplay, active_color, (x_coord, y_coord, width, height))
        if click[0] == 1 and action is not None:
            action()
    else:
        pygame.draw.rect(gameDisplay, inactive_color, (x_coord, y_coord, width, height))
